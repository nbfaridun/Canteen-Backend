...
func (r *ClientCategoryPostgres) CreateClientCategory(clientCategory *models.ClientCategory) (uint, error) {
	result := r.db.Table(models.ClientCategoryTableName).Create(clientCategory)
	if result.Error != nil {
...
func (r *ClientCategoryPostgres) GetAllClientCategories() (*[]models.ClientCategory, error) {
	...
	result := r.db.Table(models.ClientCategoryTableName).Find(&clientCategories)
	if result.Error != nil {
...
func (r *ClientCategoryPostgres) GetClientCategoryByID(id uint) (*models.ClientCategory, error) {
	...
	result := r.db.Table(models.ClientCategoryTableName).First(&clientCategory, "client_category_id = ?", id)
	if result.Error != nil {
		return nil, result.Error
	}
...
func (r *ClientCategoryPostgres) UpdateClientCategory(id uint, clientCategory *models.ClientCategory) error {
	result := r.db.Table(models.ClientCategoryTableName).Model(&models.ClientCategory{}).Where("client_category_id = ?", id).Updates(clientCategory)
	if result.Error != nil {
		return result.Error
...
func (r *ClientCategoryPostgres) DeleteClientCategory(id uint) error {
	result := r.db.Table(models.ClientCategoryTableName).Delete(&models.ClientCategory{}, "client_category_id = ?", id)
	if result.Error != nil {
		return result.Error
	} else if result.RowsAffected == 0 {
		return gorm.ErrRecordNotFound
